    ...
    const fetchContentTypes = async () => {
      ...
      const contentTypesWithPermissions = await Promise.all(
        data.map(async contentType => {
          const hasPermission = await hasPermissions(userPermissions, permissions(contentType))
      ...
      const filteredContentTypes = contentTypesWithPermissions.filter(({ hasPermission }) => hasPermission)
